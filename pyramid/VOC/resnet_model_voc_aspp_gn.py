...
def resnet_bottleneck(l, ch_out, stride, stride_first=False):
    ...
    with freeze_variables(stop_gradient=False, skip_collection=True):
        l = Conv2D('conv1', l, ch_out, 1, strides=stride if stride_first else 1, activation=tf.identity)    
    l = GroupNorm('conv1bn', l, 32)
    l = tf.nn.relu(l)
    with freeze_variables(stop_gradient=False, skip_collection=True):
        l = Conv2D('conv2', l, ch_out, 3, strides=1 if stride_first else stride, activation=tf.identity)   
    l = GroupNorm('conv2bn', l, 32)
    l = tf.nn.relu(l)
    ...
    with freeze_variables(stop_gradient=False, skip_collection=True):
        ldown =Conv2D('conv3', shortcut, ch_out / 8, 1, stride=stride if stride_first else 1, activation=tf.identity)
    ldown = GroupNorm('conv3bn', ldown, 4)
    ldown = tf.nn.relu(ldown)
    ...
    with freeze_variables(stop_gradient=False, skip_collection=True):
        lf = Conv2D('conv2f', lf, ch_out / 8, 3, stride=1 if stride_first else stride, activation=tf.identity)
    lf = GroupNorm('conv2fbn', lf, 4)
    lf = tf.nn.relu(lf)
    ...
    with freeze_variables(stop_gradient=False, skip_collection=True):
        lg = Conv2D('conv2g', lg, ch_out / 8, 3, stride=1 if stride_first else stride, activation=tf.identity)
    lg = GroupNorm('conv2gbn', lg, 4)
    lg = tf.nn.relu(lg)
    ...
    with freeze_variables(stop_gradient=False, skip_collection=True):
        lh = Conv2D('conv2h', lh, ch_out / 8, 3, stride=1 if stride_first else stride, activation=tf.identity)
    lh = GroupNorm('conv2hbn', lh, 4)
    lh = tf.nn.relu(lh)
    ...
    with freeze_variables(stop_gradient=False, skip_collection=True):
        li = Conv2D('conv2i', li, ch_out / 8, 3, stride=1 if stride_first else stride, activation=tf.identity)
    li = GroupNorm('conv2ibn', li, 4)
    li = tf.nn.relu(li)
...
def resnet_bottleneck_dilation(l, ch_out, stride, stride_first=False, dilation_rate = 1):
    ...
    with freeze_variables(stop_gradient=False, skip_collection=True):
        l = GrConv2D('conv2', l, ch_out, 3, stride=1 if stride_first else stride, dilation_rate=dilation_rate, nl=tf.identity)
    l = GroupNorm('conv2bn', l, 32)
    l = tf.nn.relu(l)
    ...
    with freeze_variables(stop_gradient=False, skip_collection=True):
        lf = GrConv2D('conv2f', lf, ch_out / 8, 3, stride=1 if stride_first else stride, dilation_rate=dilation_rate, nl=tf.identity)
    lf = GroupNorm('conv2fbn', lf, 4)
    lf = tf.nn.relu(lf)
    ...
    lg = Grresize('grdowng', ldown, [int(hi * 0.875), int(wi * 0.875)] )
    with freeze_variables(stop_gradient=False, skip_collection=True):
        lg = GrConv2D('conv2g', lg, ch_out / 8, 3, stride=1 if stride_first else stride, dilation_rate=dilation_rate, nl=tf.identity)
    lg = GroupNorm('conv2gbn', lg, 4)
    lg = tf.nn.relu(lg)
    ...
    lh = Grresize('grdownh', ldown, [int(hi * 1.16), int(wi * 1.16) ] )
    with freeze_variables(stop_gradient=False, skip_collection=True):
        lh = GrConv2D('conv2h', lh, ch_out / 8, 3, stride=1 if stride_first else stride, dilation_rate=dilation_rate, nl=tf.identity)
    lh = GroupNorm('conv2hbn', lh, 4)
    lh = tf.nn.relu(lh)
    ...
    li = Grresize('grdowni', ldown, [int(hi * 1.33), int(wi * 1.33)] )
    with freeze_variables(stop_gradient=False, skip_collection=True):
        li = GrConv2D('conv2i', li, ch_out / 8, 3, stride=1 if stride_first else stride, dilation_rate=dilation_rate, nl=tf.identity)
    li = GroupNorm('conv2ibn', li, 4)
    li = tf.nn.relu(li)
...
def aspp(l, name, ch_out):
    with tf.variable_scope(name):
        l1 = GrConv2D('aspp6', l, ch_out, 3, stride=1, dilation_rate=6)
        l2 = GrConv2D('aspp12', l, ch_out, 3, stride=1, dilation_rate=12)
        l3 = GrConv2D('aspp18', l, ch_out, 3, stride=1, dilation_rate=18)
        l4 = GrConv2D('aspp24', l, ch_out, 3, stride=1, dilation_rate=24)
    return l1 + l2 + l3 + l4
