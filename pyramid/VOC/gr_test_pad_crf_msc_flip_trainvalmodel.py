...
def main(_):
  ...
  gr_models.load_variables_from_checkpoint(sess, '../train_log_trainval/imagenet-resnet-d101-trainval/model-1187220')
  ...
  for start in xrange(0, len(df_test), batch_size):
    ...
    for id in df_test_batch['data']:
      ...
      name = id.split('/', 10)
      cv2.imwrite('prediction-val-flip-trainval/' + name[1][0:11] + '.png', pred_label) 
