...
def resnet_shortcut(l, n_out, stride, activation=tf.identity):
    ...
    if n_in != n_out:   # change dimension when channel is not the same
        l = Conv2D('convshortcut', l, n_out, 1, strides=stride, activation=tf.identity)
        l = GroupNorm('convshortcutbn', l, 32)
        return l
    else:
        return l
...
def resnet_backbone(image, num_blocks, group_func, group_func_dilation, block_func, block_func_dilation):
    with argscope(Conv2D, use_bias=False,
        ...
        logits = (LinearWrap(image)
                  .Conv2D('conv0', 64, 7, strides=2, activation=tf.identity)())
        logits = GroupNorm('conv0bn', logits, 32)
