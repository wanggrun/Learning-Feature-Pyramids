...
def main(_):
  ...
  df_test = pd.read_csv('test-continue.csv', delim_whitespace=True, header=0)
  ...
  for start in xrange(0, len(df_test), batch_size):
    ...
    for id in df_test_batch['data']:
      ...
      for sc in scs:
        ...
        if CRF:
          score_map = softmax(score_map)
          score_map = dense_crf(score_map, img_ori) 
      ...
      name = id.split('/', 10)
      cv2.imwrite('prediction-more/' + name[1][0:11] + '.png', pred_label) 
